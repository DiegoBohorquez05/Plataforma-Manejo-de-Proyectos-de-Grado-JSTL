...
<head>
    ...
    <style>
        ...
        .btn-review:disabled { background-color: #444; color: #888; cursor: not-allowed; transform: none; }
                ...
                <c:forEach var="doc" items="${docsAsignados.rows}">
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="btn-group">
                                <%-- BOTÓN VER DRIVE: Siempre habilitado --%>
                                <a href="${doc.link_drive}" target="_blank" class="btn btn-outline-light btn-sm mr-2" title="Ver en Drive">
                                    <i class="fas fa-external-link-alt"></i>
                                </a>

                                <%-- LÓGICA DE BLOQUEO: Si ya no está Pendiente, se deshabilita --%>
                                <button type="button" class="btn-review" 
                                    data-toggle="modal" 
                                    data-target="#modal${doc.id}"
                                    ${doc.estado_director != 'Pendiente' ? 'disabled title="Ya calificado"' : ''}>
                                    <c:choose>
                                        <c:when test="${doc.estado_director != 'Pendiente'}">
                                            CALIFICADO
                                        </c:when>
                                        <c:otherwise>
                                            CALIFICAR
                    ...
                    <%-- MODAL DE CALIFICACIÓN (Solo accesible si el botón no está disabled) --%>
                                    ...
                                    <div class="modal-body">
                                        <div class="form-group">
                                            ...
                                            <select name="txtEstado" class="form-control bg-black text-white border-secondary" required>
                                                ...
                                                <option value="Corregir">Requiere Correcciones</option>
                                            </select>
                                        </div>
                                        <p class="text-muted small"><strong>Nota:</strong> Una vez guardada la calificación, no podrá ser modificada.</p>
                                    </div>
                                    <div class="modal-footer border-secondary">
                                        <button type="submit" class="btn btn-warning btn-block font-weight-bold">GUARDAR CAMBIOS</button>
                                    </div>
                                </form>
                            </div>
                        </div>
